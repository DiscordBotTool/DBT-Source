module.exports = {
    ...
    html: function (insert) {
        ...
        <label>Thread Archieve Duration</label>
        <input name="threadArchieve">
        ${insert}

        <label>Thread Reason</label>
        <input name="threadReason">
        ${insert}
        `;
    },
    ...
    execute: async function (DBT, action, index, message, args, command) {
        ...
        const newThread = await channel.threads.create({
            name: DBT.var(threadName.value),
            autoArchiveDuration: DBT.var(threadArchieve),
            reason: DBT.var(threadReason.value),
